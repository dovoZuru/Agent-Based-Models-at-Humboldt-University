gardeners-own [harvesting-ticks leaves-seen leaves-harvested happy hunger-ticks]
  ...
  ask gardeners [
    if ticks mod 5 = 0 [
      if leaves-harvested >= 5 [
        ...
        set happy false
        set hunger-ticks hunger-ticks + 10  ; Sad for 5 ticks means hungry for 10 ticks
      ]
      ; Reset counters
      set leaves-seen 0
      set leaves-harvested 0
    ...
    if hunger-ticks > 0 [
      set hunger-ticks hunger-ticks - 1
    ]
  ]
end


to update-hunger-plot
  set-current-plot "Hunger Rate"
  set-current-plot-pen "Hungry Gardeners"
  plot count gardeners with [hunger-ticks > 0]
end
...
gardeners-own [harvesting-ticks leaves-seen leaves-harvested happy hunger-ticks original-color]
leaves-own [previous-color]
