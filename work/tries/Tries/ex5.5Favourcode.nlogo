...
to move
  if (rule = "Segregation") [
    ifelse (who < 3) [
      move-within-bounds -16 -2 3 16
    ] [
      ifelse (who >= 3 and who < 7) [
        move-within-bounds 0 16 -16 3
      ] [
        if (who >= 7) [
          move-within-bounds -16 17 -16 6
        ]
      ]
    ]
  ]
...
to move-within-bounds [min-x max-x min-y max-y]
  let heading-before heading
  fd 0.5
  if (xcor < (min-x + 0.5) or xcor > (max-x - 0.5) or ycor < (min-y + 0.5) or ycor > (max-y - 0.5)) [
...
to cure-plants
  let sick-leaves-in-r leaves with [color = yellow] in-radius cure-radius
  if any? sick-leaves-in-r [
    let target one-of sick-leaves-in-r
    face target
    move-to target
